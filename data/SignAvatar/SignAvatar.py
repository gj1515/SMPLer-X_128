...
class SignAvatar(torch.utils.data.Dataset):
    ...
    def __init__(self, transform, data_split):
        ...
        self.valid_sample_interval = getattr(cfg, 'SignAvatar_valid_sample_interval', 1)
        self.offset_step = getattr(cfg, 'SignAvatar_train_offset', 1)

        # Paths
        self.data_root = osp.join('C:/Users/user/Desktop/Dev/Dataset', 'SignAvatar')
        self.img_path = osp.join(self.data_root, 'images')
        self.annot_path = osp.join(self.data_root, 'annotation')
        self.json_path = osp.join(self.annot_path, 'final_ann')
        self.pkl_path = osp.join(self.annot_path, 'how2sign_pkls_cropTrue_shapeTrue')

        # Image shape (fixed for SignAvatar)
        ...
        # Format: clip_name per line (e.g., "beiObN0JB7c_1-5-rgb_front")
        ...
        if osp.exists(split_file):
            ...
            print(f'[SignAvatar] Loaded {len(self.clip_list)} clips from {split_file}')
        else:
            self.clip_list = []
            print(f'[SignAvatar] Split file not found: {split_file}')
    ...
    def load_data(self):
        """Load data from clip_list. Each clip has JSON (final_ann) and PKL (smplx)."""
        datalist = []
        original_count = 0
        sampled_count = 0
        skipped_clips = 0

        for clip_name in tqdm(self.clip_list, desc=f'[SignAvatar] Loading {self.data_split}'):
            # File paths
            json_file = osp.join(self.json_path, f'{clip_name}.json')
            pkl_file = osp.join(self.pkl_path, f'{clip_name}.pkl')

            # Skip if files don't exist
            if not osp.exists(json_file) or not osp.exists(pkl_file):
                skipped_clips += 1
                continue

            # Load JSON (keypoints, bbox, camera)
            with open(json_file, 'r') as f:
                json_data = json.load(f)

            # Load PKL (smplx params)
            with open(pkl_file, 'rb') as f:
                pkl_data = pickle.load(f)

            # Get smplx params from PKL (N, 182)
            smplx_all = pkl_data['smplx']

            # Build annotation dict by frame id
            ann_dict = {ann['id']: ann for ann in json_data['annotations']}

            # Process each frame
            for frame_idx, img_info in enumerate(json_data['images']):
                original_count += 1
                image_id = img_info['id']
                file_name = img_info['file_name']  # {clip_name}/frame_{id}.jpg

                # Image path
                img_file = osp.join(self.img_path, file_name)
                if not osp.exists(img_file):
                    continue

                # Get annotation for this frame
                ann = ann_dict.get(image_id)
                if ann is None:
                    continue

                # Get smplx params for this frame
                if frame_idx >= len(smplx_all):
                    continue
                smplx_params = smplx_all[frame_idx]

                # Check for NaN in smplx params
                if np.any(np.isnan(smplx_params)):
                    continue
                ...
                rhand_kpts = np.array(kp2d['righthand_kpts'], dtype=np.float32).reshape(-1, 3)
                face_kpts = np.array(kp2d['face_kpts'], dtype=np.float32).reshape(-1, 3)

                joint_img = self.merge_joint(body_kpts, foot_kpts, lhand_kpts, rhand_kpts, face_kpts)
                joint_conf = joint_img[:, 2].copy().reshape(-1, 1)  # save confidence before clearing
                joint_valid = (joint_conf > 0.5).astype(np.float32)
                joint_img[:, 2] = 0
                ...
                # Camera params from JSON
                cam_params = ann['camera_params']
                focal = cam_params['focal']
                princpt = cam_params['princpt']

                # Hand validity from JSON
                left_valid = ann['left_valid']
                right_valid = ann['right_valid']

                # Convert smplx_params (PKL format)
                smplx_param = self._convert_smplx_params(smplx_params, focal, princpt, left_valid, right_valid)

                # Generate unique ann_id
                ann_id = f'{clip_name}_{image_id}'
        ...
        print(f'[SignAvatar] {self.data_split}: clips={len(self.clip_list)}, skipped={skipped_clips}, '
              f'original={original_count}, sampled={sampled_count}')

        if getattr(cfg, 'data_strategy', None) == 'balance':
            print(f"[SignAvatar] Using [balance] strategy with datalist shuffled...")
    ...
    def _convert_smplx_params(self, smplx_params, focal, princpt, left_valid, right_valid):
        """Convert PKL smplx_params (182-dim array) to expected format.

        PKL smplx layout (182 dims):
            [0:3]     root_pose   (3)
            [3:66]    body_pose   (63)
            [66:111]  lhand_pose  (45)
            [111:156] rhand_pose  (45)
            [156:159] jaw_pose    (3)
            [159:169] shape       (10)
            [169:179] expr        (10)
            [179:182] trans       (3)
        ...
        return {
            'smplx_param': {
                'root_pose': smplx_params[0:3].tolist(),
                'body_pose': smplx_params[3:66].tolist(),
                'lhand_pose': smplx_params[66:111].tolist(),
                'rhand_pose': smplx_params[111:156].tolist(),
                'jaw_pose': smplx_params[156:159].tolist(),
                'shape': smplx_params[159:169].tolist(),
                'expr': smplx_params[169:179].tolist(),
                'trans': smplx_params[179:182].tolist(),
                'lhand_valid': left_valid,
                'rhand_valid': right_valid,
                'face_valid': True,
            },
            'cam_param': {
                'focal': focal,
